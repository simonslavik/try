...
import { FiSend, FiCornerUpLeft, FiX, FiSearch, FiCompass, FiPlusCircle, FiUsers, FiMessageCircle, FiBookOpen } from 'react-icons/fi';
...
const DMChat = ({ otherUser, messages, onSendMessage, auth, setMessages, dmWs, replyingTo, setReplyingTo, hasMoreMessages = false, loadingOlder = false, onLoadOlder, typingUsers = [], onTyping, friends = [], conversations = [], onSelectConversation }) => {
  ...
  if (!otherUser) {
    ...
    // Friends who don't have an existing conversation
    const conversationUserIds = new Set(conversations.filter(c => c.friend).map(c => c.friend.id));
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-900 p-4">
        <div className="max-w-lg w-full text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
            ...
            <button
              onClick={() => {
                // Focus the search input in DMSidebar
          ...
          {/* Online friends / start a conversation */}
