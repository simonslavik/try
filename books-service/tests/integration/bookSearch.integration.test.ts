...
describe('Book Search Integration Tests', () => {
  ...
  describe('GET /v1/books/search', () => {
    it('should search books by query', async () => {
      ...
      const response = await request(app).get('/v1/books/search?q=test').expect(200);

      expect(response.body.success).toBe(true);
      expect(response.body.data).toHaveLength(2);
      expect(response.body.data[0].title).toBe('Test Book 1');
      expect(GoogleBooksService.searchBooks).toHaveBeenCalledWith('intitle:test', 20);
    });

    it('should return 400 if query is missing', async () => {
      const response = await request(app).get('/v1/books/search').expect(400);

      expect(response.body.error).toContain('Query parameter');
    });

    it('should handle search errors', async () => {
      (GoogleBooksService.searchBooks as jest.Mock).mockRejectedValue(new Error('API error'));

      const response = await request(app).get('/v1/books/search?q=test').expect(500);
    ...
    it('should return empty array for no results', async () => {
      ...
      const response = await request(app).get('/v1/books/search?q=nonexistent').expect(200);
    ...
    it('should handle empty query string', async () => {
      const response = await request(app).get('/v1/books/search?q=').expect(400);
    ...
    it('should handle special characters in query', async () => {
      ...
      const response = await request(app).get('/v1/books/search?q=C%2B%2B').expect(200);

      expect(response.body.success).toBe(true);
      expect(GoogleBooksService.searchBooks).toHaveBeenCalledWith('intitle:C++', 20);
    });

    it('should handle unicode characters in query', async () => {
      (GoogleBooksService.searchBooks as jest.Mock).mockResolvedValue([]);

      const response = await request(app).get('/v1/books/search?q=日本語').expect(200);

      expect(response.body.success).toBe(true);
      expect(GoogleBooksService.searchBooks).toHaveBeenCalledWith('intitle:日本語', 20);
    });
    ...
    it('should handle very long query strings', async () => {
      ...
      const response = await request(app).get(`/v1/books/search?q=${longQuery}`).expect(200);
    ...
    it('should handle whitespace in query', async () => {
      ...
      await request(app).get('/v1/books/search?q=test%20book%20title').expect(200);

      expect(GoogleBooksService.searchBooks).toHaveBeenCalledWith('intitle:test book title', 20);
    });

    it('should handle network timeout', async () => {
      (GoogleBooksService.searchBooks as jest.Mock).mockRejectedValue(new Error('Network timeout'));
    ...
    it('should handle single result', async () => {
      ...
      const response = await request(app).get('/v1/books/search?q=single').expect(200);
    ...
    it('should handle large result sets', async () => {
      ...
      const response = await request(app).get('/v1/books/search?q=popular').expect(200);
  ...
  describe('GET /v1/books/google/:googleBooksId', () => {
    it('should get book by ID', async () => {
      ...
      const response = await request(app).get('/v1/books/google/abc123').expect(200);
    ...
    it('should return 500 for non-existent book', async () => {
      ...
      const response = await request(app).get('/v1/books/google/nonexistent').expect(500);
    ...
    it('should handle book ID with special characters', async () => {
      ...
      const response = await request(app).get('/v1/books/google/abc-123_xyz').expect(200);
    ...
    it('should handle API timeout for book lookup', async () => {
      (GoogleBooksService.getBookById as jest.Mock).mockRejectedValue(new Error('Request timeout'));

      const response = await request(app).get('/v1/books/google/timeout123').expect(500);
    ...
    it('should handle book with missing optional fields', async () => {
      ...
      const response = await request(app).get('/v1/books/google/minimal123').expect(200);
    ...
    it('should handle very long book IDs', async () => {
      ...
      const response = await request(app).get(`/v1/books/google/${longId}`).expect(200);
    ...
    it('should handle numeric book IDs', async () => {
      ...
      const response = await request(app).get('/v1/books/google/12345').expect(200);
    ...
    it('should handle book with null values', async () => {
      ...
      const response = await request(app).get('/v1/books/google/null123').expect(200);
